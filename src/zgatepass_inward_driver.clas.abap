CLASS zgatepass_inward_driver DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
*                  company_code     TYPE string
  ...
  PRIVATE SECTION.
    ...
    CONSTANTS lc_template_name TYPE string VALUE 'ZGatePass_Inward/ZGatePass_Inward'."'zpo/zpo_v2'."

ENDCLASS.



CLASS ZGATEPASS_INWARD_DRIVER IMPLEMENTATION.
  ...
  METHOD read_posts .
    ...
    INTO TABLE @DATA(it_line).
     ...
     inner join ztable_plant as b on b~plant_code = a~plant
     INNER JOIN I_CompanyCode as c on c~CompanyCode = b~comp_code
     WHERE gateentryno = @lv_gateentry
    INTO  @DATA(header_info).

    DATA address TYPE string.

    CONCATENATE header_info-b-plant_name2 header_info-b-address1 header_info-b-city header_info-b-state_name INTO address SEPARATED BY ','.

    DATA : lv_count TYPE i .
    lv_count = 1.

    DATA(lv_xml) = |<Form>| &&
                    |<CompanyName>{ header_info-c-CompanyCodeName }</CompanyName>| &&
                    |<CompAddress>{ address }</CompAddress>| &&
                     |<CompCode>{ header_info-b-comp_code }</CompCode>| &&
