CLASS zrgp_out_driver DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
*                  company_code     TYPE string
  ...
  PRIVATE SECTION.
    ...
    CONSTANTS lc_template_name TYPE string VALUE 'ZRGPOUT/ZRGPOUT'."'zpo/zpo_v2'."

ENDCLASS.



CLASS ZRGP_OUT_DRIVER IMPLEMENTATION.
  ...
  METHOD read_posts .
    ...
    SELECT  SINGLE *
   FROM zgateentryheader AS a
   INNER JOIN ztable_plant AS b ON a~plant = b~plant_code
   INNER JOIN I_CompanyCode AS c ON b~comp_code  =  c~CompanyCode
   WHERE a~gateentryno = @lv_gateentry
   INTO  @DATA(lv_headerinfo).

    DATA address TYPE string.

    CONCATENATE lv_headerinfo-b-plant_name1 lv_headerinfo-b-address1 lv_headerinfo-b-city lv_headerinfo-b-state_name INTO address SEPARATED BY ','.
    ...
    DATA(lv_xml) = |<Form>| &&
                    |<CompanyCode>{ lv_headerinfo-c-CompanyCode }</CompanyCode>| &&
                    |<CompanyName>{ lv_headerinfo-c-CompanyCodeName }</CompanyName>| &&
                    |<Companyadress>{ address }</Companyadress>| &&
                    ...
                    |<ExpectedReturnDate>{ lv_header-expectrtdate }</ExpectedReturnDate>| &&
